...
SurrealDB中的每个语句默认都在其自己的事务中运行。可以使用 `BEGIN` 语句修改这种行为，通过在一个单独的事务中运行一组语句，要么整体成功，要么整体失败。一旦事务中的所有语句都成功执行，就可以通过在事务结束时使用 xref:statements/commit.adoc[COMMIT] 语句来使所有数据修改永久化。如果事务中的任何语句遇到错误或事务被手动取消（xref:statements/cancel.adoc[CANCEL]），那么事务中所做的任何数据修改都将回滚，不会成为数据库的永久部分。
